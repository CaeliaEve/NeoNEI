...
let width = 0;
let height = 0;
let webglRenderer: NativeWorkerWebglRenderer | null = null;
...
class NativeWorkerWebglRenderer {
  ...
  static create(activeCanvas: OffscreenCanvas): NativeWorkerWebglRenderer | null {
    ...
    return new NativeWorkerWebglRenderer(gl, program, positionBuffer, colorBuffer);
  }
  ...
  render(activeWidth: number, activeHeight: number, commands: ParsedRenderCommand[]): { drawCalls: number; vertexCount: number } {
    const gl = this.gl;
...
function parseCommandBuffer(commandBuffer: ArrayBuffer, commandStride: number, count: number): ParsedRenderCommand[] {
  ...
  const result: ParsedRenderCommand[] = [];
...
async function handleRequest(message: NativeRenderRequest): Promise<NativeRenderResponse> {
  switch (message.type) {
    case "initialize": {
      ...
      webglRenderer = backend === "webgl2" ? NativeWorkerWebglRenderer.create(canvas) : null;
    ...
    case "render": {
      ...
      const parsedCommands = parseCommandBuffer(message.commandBuffer, message.commandStride, commandCount);
