...
export function getItemImageUrlFromEntity(item: {
  ...
  if (item?.preferredImageUrl && (item.preferredImageUrl.includes('/images/') || item.preferredImageUrl.includes('/assets/'))) {
...
export function getImageUrlFromFileName(imageFileName: string): string {
  ...
  if (/^https?:\/\//i.test(imageFileName) || imageFileName.startsWith('/api/images/') || imageFileName.startsWith('/assets/')) {
    return imageFileName;
  }
