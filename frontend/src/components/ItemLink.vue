...
const props = withDefaults(defineProps<{ target: Reference & { amount?: string | null }; records: Records; catalog: Catalog;
  width?: number; height?: number; animate?: boolean; compact?: boolean; note?: string; amountLabel?: string }>(),
  { width: 28, height: 28, animate: true, compact: false, note: '', amountLabel: '' });
...
const quantity = computed(() => props.amountLabel || (props.target.amount ? amount(props.target.amount) : ''));
const title = computed(() => [plain(name.value), quantity.value ? quantity.value + (props.target.kind === 'fluid' ? ' mB' : '') : '',
...
<template>
  <button type="button" class="item-link" :class="{ compact }" :title="title" :aria-label="title"
    ...
    <span v-if="compact && (amountLabel || target.amount && target.amount !== '1')" class="quantity">{{ amountLabel || target.amount }}</span>
    <span v-if="!compact" class="item-caption"><GameText :text="name" /><small v-if="quantity">{{ quantity }}{{ target.kind === 'fluid' ? ' mB' : '' }}</small></span>
  </button>
</template>
