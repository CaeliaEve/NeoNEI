...
for (const offline of [false, true]) test(`${offline ? 'offline' : 'online'} browse, search, recipes and all domain pages share the same catalog`, async ({ page, context }) => {
  ...
  await page.locator('.recipe-view .stack-slot').first().locator('.item-link').click({ button: 'right' });
  await expect(page.getByRole('button', { name: '用途', exact: true })).toHaveAttribute('aria-pressed', 'true');
  await page.getByRole('button', { name: '打开设置', exact: true }).click();
  ...
  const inputSlot = page.locator('.recipe-view .stack-slot').first();
  ...
  await inputSlot.getByRole('button', { name: '查看 3 个候选输入', exact: true }).click();
  await choices.locator('.choice-list .item-link').nth(2).click();
  await expect(inputSlot.locator(':scope > .item-link')).toHaveAttribute('title', /不消耗 · 精确匹配/);
  await inputSlot.getByRole('button', { name: '查看 3 个候选输入', exact: true }).click();
  await choices.locator('.choice-list .item-link').first().click();
  await expect(inputSlot.locator('.quantity')).toHaveText('9007199254740993');
  ...
  const portal = page.getByRole('article', { name: '物品线索 minecraft:portal', exact: true });
  await expect(portal).toContainText('任意元数据');
  await expect(portal).toContainText('资料库中无具体物品示例');
  await expect(portal.getByRole('button')).toHaveCount(0);
  const taggedClue = page.getByRole('article', { name: '物品线索 fixture:tagged', exact: true });
  await taggedClue.getByText('NBT 条件', { exact: true }).click();
  await expect(taggedClue.locator('pre')).toContainText('9223372036854775807');
  await expect(taggedClue).toContainText('fixtureClue');
  await expect(page.getByRole('article', { name: '物品线索 minecraft:stone', exact: true }).getByRole('button', { name: /Stone 石头/ })).toBeVisible();
