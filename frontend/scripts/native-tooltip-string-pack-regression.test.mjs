...
const parserSource = readFileSync(new URL('../src/workers/nativeSurfaceRuntimeParsers.ts', import.meta.url), 'utf8');
const hitTestSource = readFileSync(new URL('../src/workers/nativeSurfaceWorkerHitTest.ts', import.meta.url), 'utf8');
...
test('native runtime group hits remain expandable groups when compat entries cannot match them', () => {
  assert.match(
    controllerSource,
    /function buildSyntheticGroup\(item: Item, tooltip: NativeTooltipPayload \| null\): BrowserVariantGroup \| null/,
    'controller should synthesize BrowserVariantGroup objects from native runtime hit metadata',
  );
  assert.match(
    controllerSource,
    /kind: syntheticGroup \? "group-collapsed" : "item"/,
    'native group hits must not be downgraded to ordinary items',
  );
  assert.match(
    controllerSource,
  ...
  assert.match(
    ...
    'native runtime expanded-group panels should report the native group size',
  );
  assert.match(
    surfaceSource,
