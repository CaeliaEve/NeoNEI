...
test('native UI registry resolves UI-pack template authority with inline dynamic overlays', () => {
  const template = {
    ...
    handlerCount: 1,
    slotCount: 1,
    slots: [{ role: 'item-input', startIndex: 0, columns: 2, rows: 1, x: 45, y: 24 }],
...
test('native UI registry keeps inline and missing layouts explicit', () => {
  const inline = resolveNativeUiRuntimeSurface({
    ...
    inlineLayout: normalizeNativeUiLayoutSurface({ width: 80, height: 40, slots: [{ role: 'item-output', x: 3, y: 4 }] }),
  });
...
test('native UI registry builds exact design-space slot cells and fit matrix', () => {
  ...
  const cells = buildNativeUiSlotCells({
    slotSize: 18,
    slots: [
      { role: 'item-input', startIndex: 1, columns: 2, rows: 1, x: 45, y: 24 },
      { role: 'item-output', startIndex: 10, columns: 1, rows: 1, x: 115, y: 24 },
    ],
    resolveRoleEntries: (role) => entriesByRole.get(role) ?? [],
  });

  assert.deepEqual(cells.map((cell) => [cell.key, cell.x, cell.y, cell.entry]), [
    ['item-input:0:0', 45, 24, 'b'],
    ['item-input:0:1', 63, 24, 'c'],
    ['item-output:1:0', 115, 24, 'out'],
  ]);
