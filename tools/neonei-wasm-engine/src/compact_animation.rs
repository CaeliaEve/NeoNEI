...
const COMPACT_ANIMATION_MAGIC: &[u8; 8] = b"NEIANM1\0";
const COMPACT_ANIMATION_VERSION: u32 = 1;
const COMPACT_ANIMATION_ROW_STRIDE: u32 = 5;
const COMPACT_ANIMATION_FRAME_STRIDE: u32 = 2;
const COMPACT_ANIMATION_HEADER_BYTES: usize = 32;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct CompactAnimationHeader {
    pub item_count: u32,
...
pub fn parse_compact_animation_header(bytes: &[u8]) -> Option<CompactAnimationHeader> {
    if bytes.len() < COMPACT_ANIMATION_HEADER_BYTES {
        return None;
    }
    if bytes.get(0..8)? != COMPACT_ANIMATION_MAGIC {
        return None;
    }
    let version = read_u32_le(bytes, 8)?;
    if version != COMPACT_ANIMATION_VERSION {
        return None;
    }
    ...
    if row_stride != COMPACT_ANIMATION_ROW_STRIDE || frame_stride != COMPACT_ANIMATION_FRAME_STRIDE
    {
        return None;
    }
...
mod tests {
    ...
    fn parses_empty_animation_header() {
        ...
        bytes.extend_from_slice(&COMPACT_ANIMATION_ROW_STRIDE.to_le_bytes());
        bytes.extend_from_slice(&COMPACT_ANIMATION_FRAME_STRIDE.to_le_bytes());
        let header = parse_compact_animation_header(&bytes).expect("header");
