...
test('runtime manifest ETag and contract envelope are delivery-service owned', () => {
  ...
  assert.match(deliveryAbiSource, /RUNTIME_MANIFEST_DELIVERY_DESCRIPTORS/);
  assert.match(deliveryAbiSource, /validateAndFreezeRuntimeManifestDeliveryDescriptors/);
  assert.match(deliveryAbiSource, /Runtime manifest delivery descriptor must not be null/);
  assert.match(deliveryAbiSource, /Missing runtime manifest delivery profile/);
  assert.match(deliveryAbiSource, /const RUNTIME_MANIFEST_DELIVERY_CONTRACTS/);
  assert.match(deliveryAbiSource, /'neonei\/runtime-manifest\/current'/);
  assert.match(deliveryAbiSource, /'\/runtime\/contracts'/);
  assert.match(deliveryAbiSource, /'neonei\/api-v1\/runtime-manifest\/v1'/);
  assert.match(deliveryAbiSource, /'\/api\/v1\/runtime\/contracts'/);
