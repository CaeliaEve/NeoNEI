...
当前契约为 0.10.0，source/catalog 修订 10。配方候选各自拥有匹配规则，网格独立于截图；魔法成本和研究通过 Related.topics 统一加载图标与资料引用。视图 cost 元素可跳转到对应要素。结构 variants 列表为各组参数保存构建引用或失败原因，builds/blocks/models 集合保留实际方块、完整NBT、控制器坐标和外观；几何共用Cell.index。旧离线副本提示下载当前资料，不进行旧格式转换。
